...
COMMENT ON COLUMN sys_dict.is_deleted IS '逻辑删除(1-已删除 0-未删除)';
...
COMMENT ON COLUMN sys_dict_item.dict_code IS '字典编码，与 sys_dict.dict_code 对应';
COMMENT ON COLUMN sys_dict_item.value IS '字典项值';
COMMENT ON COLUMN sys_dict_item.label IS '字典项标签';
COMMENT ON COLUMN sys_dict_item.tag_type IS '标签样式(primary-主要 success-成功 warning-警告 danger-危险 info-信息，空为无样式)';
COMMENT ON COLUMN sys_dict_item.status IS '状态(1-启用 0-停用)';
...
COMMENT ON COLUMN sys_dict_item.is_deleted IS '逻辑删除(1-已删除 0-未删除)';
...
COMMENT ON COLUMN gen_table.page_type IS '页面类型(classic|crud)';
