...
from app.modules.codegen.schemas import GenConfigForm, PreviewVO, TableVO, TableQuery


class CodegenService:
    def __init__(self, db: AsyncSession):
        self.db = db
        self._config_cache: dict[str, GenConfigForm] = {}
    ...
    async def get_gen_config(self, table_name: str) -> GenConfigForm:
        ...
        if table_name in self._config_cache:
            return self._config_cache[table_name]
    ...
    async def save_gen_config(self, table_name: str, form: GenConfigForm) -> None:
        """将代码生成配置存入进程内缓存。"""
        self._config_cache[table_name] = form

    async def delete_gen_config(self, table_name: str) -> None:
        """删除进程内缓存的代码生成配置。"""
        self._config_cache.pop(table_name, None)
