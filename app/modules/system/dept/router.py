...
from app.modules.system.dept.schemas import DeptCreate, DeptUpdate
from app.modules.system.dept.service import DeptService
...
async def create_dept(form: DeptCreate, db: AsyncSession = Depends(get_db)):
    vo = await DeptService(db).create(form)
    return Result(data=vo)


@router.put("/{dept_id}", summary="更新部门", dependencies=[Depends(require_perm("sys:dept:update"))])
async def update_dept(dept_id: int, form: DeptUpdate, db: AsyncSession = Depends(get_db)):
    form.id = dept_id
...
async def delete_depts(ids: str, db: AsyncSession = Depends(get_db)):
