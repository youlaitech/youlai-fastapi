...
from app.common.constants import RedisConstants, SystemConstants
from app.core.redis import get_redis
from app.framework.security.schemas import CaptchaResult
...
class CaptchaService:
    ...
    async def generate(self) -> CaptchaResult:
        ...
        await redis_client.setex(
            f"{RedisConstants.CAPTCHA_PREFIX}{captcha_id}",
            SystemConstants.CAPTCHA_TTL,
            code.lower(),
        )
    ...
    async def verify(self, captcha_id: str, code: str) -> bool:
        """校验验证码 — 一次性使用。"""
        redis_client = await get_redis()
        key = f"{RedisConstants.CAPTCHA_PREFIX}{captcha_id}"
